...
def tty2img(
    ...
    for line in screen.buffer:
        ...
        for char in sorted(screen.buffer[line].keys()):
            ...
            if freetype and not _get_char_index(font[1], cData.data):
                for fname in fallbackFonts:
                    font = _get_fallback_font(fname, cData.data, fontSize)
                    if font:
                        break
                else:
                    if logFunction:
            ...
            if cData.underscore or cData.strikethrough:
                _flushChars(draw, point)
                if cData.underscore:
            ...
            # draw text
            _writeChar(draw, point, cData.data, fill=fgColor, font=font[0])
...
@functools.cache
def _get_fallback_font(family, data, fontSize):
    font = None
    for ff in fclist.fclist(family=family, charset=hex(ord(data))):
        font = [ ImageFont.truetype(ff.file, fontSize), None ]
    return font


@functools.cache
def _get_char_index(font, char):
    return font.get_char_index(char)
...
def _writeChar(draw, point, data, *, fill, font):
    global _fill, _font, _data

    if (_fill != fill or _font != font):
        if (_data):
            draw.text(point, _data, fill=_fill, font=_font)
            point[0] += _get_textsize(draw, _data, _font)[0]
...
@functools.cache
def _get_textsize(draw, data, font):
    return draw.textsize(data, font)
